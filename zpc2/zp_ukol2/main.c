...
void form_operation(Bitset *left, Bitset *right, char (*bin_op)(char, char), char (*un_op)(char)){
	for(int i = 0; i < left->data_len; i++){
		...
		else if(bin_op == and && un_op != neg) left->data[i] = 0;
	}
}

void form_intersection(Bitset *left, Bitset* right){ form_operation(left, right, and, id);}
void form_union(Bitset *left, Bitset* right) {form_operation(left, right, or, id);}
void form_difference(Bitset *left, Bitset* right) {form_operation(left, right, and, neg);}

Bitset* greater(Bitset* a, Bitset* b) {return a->data_len > b->data_len ? a : b;}
Bitset* lesser(Bitset* a, Bitset* b) {return a->data_len > b->data_len ? b : a;}
...
Bitset *set_operation(Bitset *left, Bitset *right, char (*bin_op)(char, char), char (*un_op)(char), Bitset* (*size_op) (Bitset*, Bitset* )){
	...
	for(int i = 0; i < chosen->data_len; i++){
		if(left->data_len >= i + 1 && right->data_len >= i + 1) array[i] = bin_op((unsigned char)left->data[i],  (unsigned char) un_op(right->data[i]));
		else array[i] = greater(left,right)->data[i]; 
	}	
	int size = 0;
	for(int i = 0; i < chosen->size;i++) if( ((1 << (i % 8))  & array[i / 8]) != 0) size++;
	...
	while( (c = fgetc(stream)) != EOF){
		...
		if(c == '\n'){
			...
			if(max_size){
				...
				bitsets_len++;
				//if(first_time) 
				//bitsets = (Bitset**) realloc(bitsets, sizeof(bitsets) + sizeof(Bitset*));
				bitsets = (Bitset**) malloc(sizeof(Bitset*));
				printf("sukas\n");
				bitsets[bitsets_len - 1] = create_bitset_with_values(max(array, size), array, size);
