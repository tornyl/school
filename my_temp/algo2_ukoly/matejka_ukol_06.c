...
Branch* rotate_right(Branch *root){
	...
	if(root->right) r = root->right->bf;

	root->bf  = l - r;
	
	r = root->bf;
	if(x->left) l = x->left->bf;
	else l = 0;
	x->bf  = l - r;
	//set_right-child(x, r);
	x->right = root;
...
Branch* rotate_left(Branch *root){
	...
	if(root->left) l = root->left->bf;

	root->bf = l - r;
	l = root->bf;
	if(x->right) r = x->right->bf;
	else r = 0;
...
void complete_rotation(Tree *t, Branch *branch, Branch *parent){
	...
	else if(parent->left && (parent->left == branch->left || parent->left == branch->right)){parent->left = branch;}
	else if(parent->right && (parent->right == branch->left || parent->right == branch->right)){parent->right = branch;} 
