...
(defmethod rotate ((tr triangle) phi c)
	(rotate (vertex-a tr) c phi)
	(rotate (gvertex-b tr) c phi)
	(rotate (vertex-c tr) c phi))

(defmethod scale ((tr triangle) coef c)
	(move (vertex-a tr) coef c)
	(move (vertex-b tr) coef c)
	(move (vertex-c tr) coef c))
...
(defmethod set-mg-params ((tr triangle) mgw) 
  (mg:set-param mgw :foreground (color tr)) 
  (mg:set-param mgw :thickness (thickness tr))
  (mg:set-param mgw :filledp (filledp tr))
  (mg:set-param mgw :closedp (closedp tr))
  tr)
