...
(defmethod initialize-instance ((smp semaphore) &key)
	(call-next-method)
	(change-semaphore-type smp :vehicle))
...
(defmethod change-semaphore-type ((smp semaphore) type)
	(set-semaphore-type smp type)
	(make-semaphore smp))

(defmethod make-semaphore ((smp semaphore))
	(when (eql (semaphore-type smp) :pedestrian)
			(set-phase-count smp 2)
			(set-stages smp (list (list 0 1) (list 1 9)))
...
(defmethod initialize-instance ((cr crossroads) &key)
	(call-next-method)
	(let ((smp1 (move (change-semaphore-type (make-instance 'semaphore) :vehicle) 0 0))
			(smp2 (move (change-semaphore-type (make-instance 'semaphore) :vehicle) 100 0))
			(smp3 (move (change-semaphore-type (make-instance 'semaphore) :vehicle) 200 0)))
	(set-items cr (list smp1 smp2 smp3))
	(set-program cr (list (list 1 0 2) (list 5 0 1) (list 3 3 1)))))
