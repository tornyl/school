...
(defclass triangle2 (polygon)

(defmethod intialize-instance ((tr trinagle2)
(defmethod get-vertex-a ((tr triangle2))
	(slot-value tr 'vertex-a))
(defmethod get-vertex-b ((tr triangle2))
	(slot-value tr 'vertex-b))
(defmethod get-vertex-c ((tr triangle2))
	(slot-value tr 'vertex-c))


(defmethod move ((tr triangle2) dx dy)
	(move (get-vertex-a tr) dx dy)
	(move (get-vertex-b tr) dx dy)
	(move (get-vertex-c tr) dx dy))

(defmethod rotate ((tr triangle2) phi c)
	(rotate (get-vertex-a tr) c phi)
	(rotate (get-vertex-b tr) c phi)
	(rotate (get-vertex-c tr) c phi))

(defmethod scale ((tr triangle2) coef c)
	(move (get-vertex-a tr) coef c)
	(move (get-vertex-b tr) coef c)
	(move (get-vertex-c tr) coef c))
...
(defmethod rotate ((tr triangle) phi c)
	(rotate (get-vertex-a tr) c phi)
	(rotate (get-vertex-b tr) c phi)
	(rotate (get-vertex-c tr) c phi))

(defmethod scale ((tr triangle) coef c)
	(move (get-vertex-a tr) coef c)
	(move (get-vertex-b tr) coef c)
	(move (get-vertex-c tr) coef c))
