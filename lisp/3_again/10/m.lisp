...
;[object add "ERROR" :value {(arg1) (lambda (arg1(error "~a" [selfarg1])}]
...
;[pair add "IS-LIST" :value 
[pair add "IS-LIST" :value true]

;[lobby add "IS-LIST" :value { () [[self is empty-pair] or [self is pair]]}] 

;[lobby remove pair-1]
[lobby add "PAIR-1" :value  [lobby cons 1 :cdr [lobby cons 2 :cdr [lobby cons 3 :cdr empty-pair]]]]

; test
;[[pair-1 is-list] name]
;[[1 is-list] name]

;[lobby add "FC1" :value {(arg1) [arg1 print]
;										  [fc1 [arg1 - 1]]}]
...
[pair add "BROADCAST" :value {(arg1) 
												 ["OJ" print]
												 arg1
												 [lobby arg1 [self car]]
												 ;[[arg1 eql object] if-true [[super self] arg1 [self car]] :else  [{} arg1 [self car]]]
												 ;[[self cdr] broadcast {(s) [[{} arg1] s]}  ]}]
												 ;[[self cdr] broadcast  {(s) [{} s]} ]}]
												 [[self cdr] broadcast arg1 ]}]
[empty-pair add "BROADCAST" :value {(arg1) empty-pair}]
